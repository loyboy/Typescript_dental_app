      ...
      <div className="calendar-component container-fluid">
              ...
              return (
                <Col key={year} span={6} className="centered">
                  <a
                    ...
                    className={
                      (calendar.selectedYear === year ? "selected" : "") +
                      (calendar.currentYear === year ? " current" : "")
                    }
                  >
                    {year}
                  </a>
              ...
              return (
                <Col key={monthShort} sm={2} xs={4} className="centered">
                  <a
                    ...
                    className={
                      (calendar.selectedMonth === index ? "selected" : "") +
                ...
                return (
                    ...
                    onClick={() => {
                      ...
                      setTimeout(() => {
                        scroll(
                          0,
                          this.findPos(
                            document.getElementById("day_" + day.dateNum)
                          )
                        );
                      }, 0);
                    }}
                  ...
                  >
                    <div className="day-name">
                      {text(
                        day.weekDay.dayLiteralShort.substr(0, 2).toUpperCase()
                      )}
                    </div>
                    <a
                      className={
            ...
            <div>
              {calendar.selectedMonthDays.map(day => {
                const number = appointments.appointmentsForDay(
                  ...
                  day.dateNum,
                  undefined,
                  this.showAll ? undefined : user.currentUser._id
                ).length;
                return (
                  <div
                    key={day.dateNum}
                  ...
                  >
                    <div className={"info-row appointments-num num-" + number}>
                      {number}
                    </div>
                  </div>
                );
            ...
            <Row>
              ...
              <Col sm={12} md={18} xs={0} className="filter">
                <TextField
                  ...
                  onChange={(ev, newVal) => (this.filter = newVal!)}
                />
              </Col>
            </Row>
          </div>
                ...
                <div
                  ...
                  className={
                    ...
                    (user.currentUser.onDutyDays.indexOf(
                      ...
                      ? " holiday"
                      : "") +
                    (calendar.selectedDay === day.dateNum ? " selected" : "")
                  }
                  onClick={() => {
                    calendar.selectedDay = day.dateNum;
                  }}
                  style={{
                    width:
                      (100 / calendar.selectedWeekDays.length).toString() + "%"
                  }}
                >
                  <h4>
                    <b>{day.dateNum}</b>
                    ...
                    .map(appointment => {
                      return (
                        ...
                        >
                          ...
                          <PatientLinkComponent
                            id={(appointment.patient || new Patient())._id}
                          />
                          <PatientFinanceComponent
                            id={(appointment.patient || new Patient())._id}
                          />
                          {appointment.operatingStaff.map(operator => {
                            return (
                              <div key={operator._id} className="m-t-5 fs-11">
                                <Icon iconName="Contact" /> {operator.name}
                              </div>
                            );
                          })}
                        </div>
        ...
        {this.appointment ? (
          <AsyncComponent
            key="ae"
            loader={async () => {
              const AppointmentEditorPanel = (
                await import("./appointment-editor")
