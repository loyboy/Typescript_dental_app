	...
	return new Promise((resolve, reject) => {
		...
		Http.onreadystatechange = function (e) {
			if (this.readyState === 4) {
				...
				insurances.list = demoData.staffData.staffMembers.list.map(
					(x: any) => new Insurance(x)
				);
				expenses.list = demoData.expenseData.expenses.list.map(
					(x: any) => new Expense(x)
				);
				...
				expenses.list.forEach((expense, i) => {
					expenses.list[i].price = randomInRange(1,8);
					expenses.list[i].name = randomAddress();
					expenses.list[i].description = randomAddress();
					expenses.list[i].date = randomDate();
					expenses.list[i].type = "expenses";
					expenses.list[i].repeat = "0";
				});
